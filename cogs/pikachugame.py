...
class Pikachugame(commands.Cog):
    ...
    async def pika_join(self, interaction: discord.Interaction):
        ...
        self.cursor.execute("SELECT COUNT(*) FROM users WHERE id = ?", (id,))
    ...
    async def pika_status(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f"目前輪到： <@{self.find_player_by_turn(now)[0]}>({now}): **{self.find_player_by_turn(now)[2]}**")
